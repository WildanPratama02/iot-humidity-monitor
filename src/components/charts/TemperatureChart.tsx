...
export function TemperatureChart({ data }: TemperatureChartProps) {
  // --- KONSTANTA SETTING ---
  const DOMAIN_MAX = 50;  // Ujung Atas Grafik
  const DOMAIN_MIN = 0;   // Ujung Bawah Grafik
  const THRESHOLD = 25;   // Batas Ambang
  ...
  const chartData = todayData.map(item => [
    ...
    parseFloat(item.temp.toString()) // Nilai temperature untuk y-axis
  ]);
  ...
  const options: Highcharts.Options = {
    ...
    chart: {
      ...
      marginLeft: 50, // Margin kiri untuk Y axis
        ...
        return `
          <div class="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
            ...
            <div class="flex items-center gap-2">
              <div class="w-3 h-3 rounded-full ${isAbove ? 'bg-red-500' : 'bg-blue-500'}"></div>
              <p class="text-sm font-medium">
                <span class="font-bold ${isAbove ? 'text-red-600' : 'text-blue-600'}">
                  ${value.toFixed(1)}°C
                </span>
                <span class="text-gray-500 ml-1">
                  ${isAbove ? ' (⚠️ Above Max)' : ' (Normal)'}
                </span>
              </p>
            </div>
          </div>
    ...
    plotOptions: {
      line: {
        ...
        threshold: THRESHOLD,
        negativeColor: '#3b82f6', // Warna untuk nilai di bawah threshold (Biru)
        color: '#ef4444' // Warna untuk nilai di atas threshold (Merah)
      }
    },
    ...
    yAxis: {
      min: DOMAIN_MIN,
      max: DOMAIN_MAX,
      tickAmount: 5, // Jumlah ticks yang diinginkan
